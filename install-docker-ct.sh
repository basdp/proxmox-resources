...
} | progressbox "Setting up account..."
...
printf "Welcome to BasdP's Docker Container Terminal (CT) environment.\n"
printf "\n"
EOF
...
cat <<EOF > /etc/update-motd.d/30-list-projects
#!/bin/bash
# This file is managed by BasdP's Docker CT environment
# Do not edit this file, it will be overwritten on update
# This file is used to display the list of containers
...
if [ "\$(docker compose ls --format json | jq length)" -eq "0" ]; then
    printf " No docker compose projects running.\n"
    printf "\n"
    exit 0
fi

printf " Currently running docker compose projects:\n\n"
